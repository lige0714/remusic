...
public class MainFragmentAdapter extends RecyclerView.Adapter<MainFragmentAdapter.ItemHolder> {
    ...
    @Override
    public void onBindViewHolder(final ItemHolder itemHolder, int i) {  // recyclerView 初始化元素
        switch (getItemViewType(i)) {
            ...
            case 1:  //歌曲
                ...
                if (createdExpanded && playlist.author.equals("local")) {  //创建的歌单
                ...
                if (collectExpanded && !playlist.author.equals("local")) {//收藏得歌单
                    if (playlist.albumArt != null)
                        itemHolder.albumArt.setImageURI(Uri.parse(playlist.albumArt));
                    itemHolder.title.setText(playlist.name);
                    itemHolder.songcount.setText(playlist.songCount + "首");
    ...
    private void setOnListener(ItemHolder itemHolder, final int position) {
        switch (position) {
            ...
            case 1:
                itemHolder.itemView.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {//最近播放
                        final Handler handler = new Handler();
                        handler.postDelayed(new Runnable() {
                            @Override
                            public void run() {
            ...
            case 2:
                itemHolder.itemView.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {  //下载管理
//                        final Handler handler = new Handler();
//                        handler.postDelayed(new Runnable() {
//                            @Override
//                            public void run() {
...
//                        }, 60);
    ...
    private void setSectionListener(final ItemHolder itemHolder, int position) {
        itemHolder.sectionMenu.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(mContext, PlaylistManagerActivity.class);  // 歌单管理 -删除
                mContext.startActivity(intent);
            }
        });
    }
    ...
    public int getItemViewType(int position) {
        ...
        if (itemResults.get(position) instanceof MainFragmentItem)  // 菜单  instanceof  测试一个对象是否为一个类的实例
            return 0;
        if (itemResults.get(position) instanceof Playlist) { //歌曲
            return 1;
        }
